...
def list_regions(
    ...
    Note: For performance reasons, either 'q' (search query) or 'state_code'
    should be provided when fetching large numbers of regions. Unfiltered
    requests are limited to 1000 results.
    """
    repository = RegionRepository(session)
    normalized_query = q.strip() or None if q is not None else None

    # If no search query or filter is provided, warn about performance
    if normalized_query is None and state_code is None and limit > 100:
        logger.warning(
            "Large unfiltered region listing requested",
            extra={"event": "regions_list_unfiltered_large_request"},
        )
