...
def upsert_regions(session: Session, rows: Iterable[dict[str, object]]) -> None:
    ...
    for row in rows:
        ...
        region_row["slug"] = slugify_region_name(str(region_row["name"]))
...
def _fetch_features_all(type_name: str, count: int = WFS_PAGE_SIZE) -> list[dict[str, Any]]:
    ...
    while True:
        ...
        if not page:
            break
        out.extend(page)
        logger.info("WFS %s: %s Features geladen (startIndex=%s)", type_name, len(page), start_index)
        if len(page) < count:
            break
        start_index += count
    return out
...
def _build_sparql_query(article_titles: list[str]) -> str:
    ...
    title_values = " ".join(f"{json.dumps(title)}@{settings.wikipedia_language}" for title in article_titles)
    return f"""
    ...
    with httpx.Client(timeout=120, follow_redirects=True, headers=headers) as client:
        ...
        for title_batch in _chunked(titles, WIKIDATA_BATCH_SIZE):
            ...
            for attempt in range(1, WIKIDATA_REQUEST_RETRIES + 1):
                ...
                delay_seconds = float(retry_after) if retry_after else WIKIDATA_RETRY_DELAY_SECONDS * attempt
                logger.warning(
                ...
                logger.warning("Wikidata batch skipped after repeated transient failures (%s titles)", len(title_batch))
                continue
...
def enrich_rows_with_wikidata(rows: list[dict[str, object]]) -> None:
    ...
    try:
        initial_title_to_ags = {_wikipedia_title_candidates(row)[0]: str(row["ars"]) for row in rows}
...
def fetch_bkg_regions_from_postgis(session: Session) -> list[dict[str, object]]:
    ...
    geometry_flavour = _resolve_geometry_flavour(session, schema, table, settings.bkg_geometry_flavour)
    population_column = next(
        (column for column in ["ewz", "ew", "einwohner", "population"] if column in available_columns),
        None,
    )
    area_column = next(
...
def main() -> None:
    ...
    if not rows:
        logger.error("BKG-WFS Gemeindeimport fehlgeschlagen. Keine Demo-Daten mehr als Fallback.")
        return
